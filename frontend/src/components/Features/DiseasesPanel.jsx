...
const DiseasesPanel = () => {
    ...
    return (
        ...
        <div className={`absolute right-6 top-24 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 ${isMinimized ? 'w-12' : 'w-96'}`}>
            <div className="p-4 bg-mycol-brunswick_green text-white flex justify-between items-center">
                {!isMinimized && (
                    <div>
                ...
                {isMinimized && <Leaf className="w-5 h-5 mx-auto" />}
                <button 
                    onClick={toggleMinimize}
                    className="p-1 rounded-full hover:bg-mycol-brunswick_green/80 transition-colors"
                    title={isMinimized ? "Expand panel" : "Minimize panel"}
                >
                    <X className={`w-5 h-5 ${isMinimized ? 'transform rotate-45' : ''}`} />
                </button>
            </div>
            {!isMinimized && (
